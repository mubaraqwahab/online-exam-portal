<div class="bg-light border-right" id="sidebar-wrapper">
  <div class="sticky-top pt-2">
    <h5 class="sidebar-heading">Dashboard</h5>
    <div class="list-group list-group-flush">
      <a href="#" class="list-group-item list-group-item-action bg-light" id="assessment">Assessment</a>
      <a href="#" class="list-group-item list-group-item-action bg-light" id="my-quiz">My Quiz</a>
      <a href="#" class="list-group-item list-group-item-action bg-light" id="exam">Exam</a>
      <a href="#" class="list-group-item list-group-item-action bg-light" id="create-exam">Create Exam</a>
    </div>
  </div>
</div>